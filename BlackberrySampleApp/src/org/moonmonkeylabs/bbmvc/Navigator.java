...
/**
 * The Navigator class is used to handle all navigation for MVC application. It
 * keeps track of all Controller, the navigation history and pops and pushes
 * screen on the display stack
 * 
 */
public final class Navigator extends UiApplication {
    ...
    /**
     * The getInstance static method returns a singleton instance of Navigator.
     ...
     * @throws Exception
     *             It throws an exception if the Controller is null or already
     *             exists
     * 
     */
    public void addController(Controller controller) throws Exception {
        ...
        if (controller != null) {
            ...
            // Checks if a controller with the same controller id is already in
            // the cache
            if (!_controllerCache.containsKey(controllerId))
                _controllerCache.put(controllerId, controller);
            else
                throw new Exception(
                        "Already contrains a conroller with this name");
        } else
            throw new Exception("Argument is null");
    }

    /**
     * The removeController methods removes a controller from the controller
     * cache.
     * 
     * @param controller
     *            The Controller to be removed
     * @throws Exception
     *             Throws an exception if the Controller is null or not in the
     *             cache
     */
     ...
     * 
     * @param controllerId
     *            The id of the Controller to be removed.
     * @throws Exception
     *             Throws an exception if the id is zero or the controller is
     *             not in the cache
     */
    public void removeController(int controllerId) throws Exception {
        if (controllerId != 0) {
            ...
            if (_controllerCache.containsKey(controllerId)) {
                ...
                _history.removeController((Controller) _controllerCache
                        .get(controllerId));
     ...
     * It uses the controller id to look up the Controller in the cache, updates
     * the navigation history and pushes and pops screens of the display stack.
     * 
     * @param controllerId
     *            The id of the Controller to navigate to
     * @throws Exception
     *             Throws an exception if the id is less than zero or the
     *             controller is not in the cache
     */
    ...
    /**
     * The navigate method is responsible for controlling which screen is show.
     * It uses the Controller to look up the Controller in the cache, updates
     * the navigation history and pushes and pops screens of the display stack.
     * 
     * @param controller
     *            The Controller to navigate to
     * @param parameters
     *            Any initialization parameters that need to be passed to the
     *            Controller
     * @throws Exception
     *             Throws an exception if the id is less than zero or the
     *             controller is not in the cache
     ...
     *             controller is not in the cache
     */
    public void navigate(Controller controller, Object[] parameters)
            throws Exception {
        if (controller != null) {
     ...
     *             controller is not in the cache
     */
    public void navigate(int controllerId, Object[] parameters)
            throws Exception {
        if (controllerId > 0) {
            if (_controllerCache.containsKey(controllerId)) {
                Controller cachedController = (Controller) _controllerCache
                        .get(controllerId);
                ...
                if (cachedController != currentController) {
                    ...
                    // The previous screen is poped here because using the
                    // navigator, there is the case
                    // where the screen is shown again and if it is already on
                    // the stack, this causes
                    // an exception since a screen cannot be pushed twice.
                    if (currentController != null)
                    	popScreen(currentController.getView().getViewScreen());
                    // *****TP: changed to not pop the screen - it was causing
                    // problems with popscreens
                    // Have to look intot he exception with the double pushing.
                    // Posibly it just has to be pop and push
                    // and the back has to be modified so that if the desired
                    // screen is not below a back action
    ...
    public void goBack(Object[] parameters) {
        if (_history.canGoBack()) {
            ...
            // Because the old/previous screen is always poped in the navigator
            // method, it has
    ...
    /**
     * The hasController method checks if a Controller with this id exists in
     * the Controller cache
     * 
     * @param controllerId
     *            The id of the Controller is question
    ...
    /**
     * The getController accessor method returns the controller with the
     * respective id
     * 
     * @param controllerId
     *            The id of the Controller to be retrieved
    ...
    /**
     * The getCurrentController accessor method returns the current controller.
     * 
     * @return The Controller instance of the current controller, null if not
     *         current controller exists
     */
    ...
    /**
     * The removeFromHistory method completely removes a controller from the
     * history and display stack
     * 
     * @param controller
     *            The Controller to be removed
     */
    ...
    /**
     * The isPreviousController method checks if the previous has a specific id.
     * 
     * @param controllerId
     *            The id of the potential previous Controller
     * @return True if indeed the previous controller has the id, otherwise
     *         false
     */
